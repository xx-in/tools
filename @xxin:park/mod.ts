...
async function downloadImage(imageName: string, outputFilename?: string): Promise<void> {
  ...
  try {
    ...
    const response = await fetch("https://city189.cn:1091/", {
      ...
      redirect: "manual" // 手动处理可能存在的 302 重定向
    });
    ...
    const finalFilename = outputFilename || matchedFilename || "bundle.tar";
    const file = await Deno.open(finalFilename, { write: true, create: true, truncate: true });
