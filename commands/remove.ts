  ...
  program
    ...
    .action(async (paths: string[]) => {
      if (!paths || paths.length === 0) {
        console.error(c.error("❌ 请提供要放入回收站的路径或通配符规则"));
        return;
      }
      ...
      if (notFoundPaths.length > 0) {
        for (const path of notFoundPaths) {
          console.error(c.error(`❌ 路径不存在: ${path}`));
        }
      }

      if (resolvedPathsToDelete.size === 0) {
        console.log(c.warn("ℹ️ 未能找到任何与指定条件匹配的文件或目录。"));
        return;
      }
      ...
      try {
        ...
        await trash(Array.from(resolvedPathsToDelete), { glob: false });

        for (const path of resolvedPathsToDelete) {
          console.log(c.dim(`  - 已移入垃圾箱: ${path}`));
        }
